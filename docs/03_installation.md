...
| `ml/train.py`/`ml/register_model.py` échoue avec `Detected out-of-date database schema` | `mlflow.db` a été créé/migré par une version de `mlflow` plus ancienne que celle pinnée dans `requirements.txt` (`2.22.5`) — arrive si l'environnement local n'a pas été réinstallé depuis un moment | Sauvegarder `mlflow.db` (`cp mlflow.db mlflow.db.bak`), puis `mlflow db upgrade sqlite:///mlflow.db` (migration additive, aucune perte de données constatée) |
